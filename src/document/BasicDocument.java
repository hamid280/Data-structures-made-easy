...
public class BasicDocument extends Document 
{
	...
	public int getNumSyllables()
	{
        ...
        //where any vowels occurs whether small or big
        int size_syllables=getTokens("[AEIOUYaeiouy]+").size();

        //where the word ends with E but is not preceded by a vowel(big or small)
        int size_ending_with_e = getTokens("[^AEIOUYaeiouy]e\\b").size();

        //where the vowel e occurs as the only vowel
        int size_lone_e = getTokens("[ .?!,(][^AEIOUYaeiouy]+[e][ .?!,)]").size();

        int size = size_syllables-size_ending_with_e+size_lone_e;
        if(size<0)
            {
                size=0;
            }
